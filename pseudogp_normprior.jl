...
function norm_prior(t)
    lp = sum(logpdf(Normal(0.5, 1), t))
    return lp
end
...
function acceptance_ratio_normprior(X, tp, t, tau_prop, tau, 
    lambda_prop, lambda, 
    s_prop, s, gamma, df)
    """ 
    ...
    t_prior = norm_prior(tp) - norm_prior(t)
...
function pseudogp_student_normprior(X, n_iter, burn, thin, 
    ...
    for i in 1:n_iter
        ...
        alpha = acceptance_ratio_normprior(X, t_prop, t, 
                                tau_prop, tau,
                                lambda_prop, lambda, 
                                sigma_prop, sigma, 
                                gamma, df)
