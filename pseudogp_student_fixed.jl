...
function acceptance_ratio_fixed(X, tp, t, tau, lambda, 
    s, r, gamma, df)
    """ 
...
function pseudogp_student_fixed(X, n_iter, burn, thin, 
    tau, t, tvar, lambda, s; 
    ...
    @assert P == 2 # for now
    @assert length(lambda) == length(s) == P
    @assert burn < n_iter
    @assert length(t) == N
    @assert size(X) == size(tau)
    ...
    for i in 1:n_iter
        ...
        alpha = acceptance_ratio_fixed(X, t_prop, t, 
                                tau, lambda, s,
                                r, gamma, df)
